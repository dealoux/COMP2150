...
abstract class Transaction {
   ...
   public Transaction(String id, int clientID, long time) {
      this.id = id;
   ...
   /**
    * Remove a quantity of a given item from the transaction.
    *
    * @param  itemKey key ID of the inventory item to remove
    * @param  quantity the quantity of the item
    * @return total quantity removed or -1 if nothing removed
    */
   public int removeItem(String itemKey, int quantity) {
      int result = -1;

      if(quantity > 0){
         TransactionUnit unit = units.get(itemKey);

         if (unit != null) {
            quantity = Math.min(quantity, unit.getQuantity());

            unit.changeQuantity(unit.getQuantity() - quantity);

            if (unit.getQuantity() <= 0) {
               units.remove(unit.getItem().getCode());
            }
   ...
   public Map<String, TransactionUnit> getUnits() {
      return units;
   }
   ...
   public abstract A4POSServer.TransactionType getType();
   ...
   public String toString() {
      int cost = totalCost();
      return "ID: " + id + " Client ID: " + clientID + " time: " + time + "\n items: " + units + "\n value: $" + (cost / 100) + "." + (cost % 100 < 10 ? "0" : "") + (cost % 100);
   }
}