...
class A4ModelPOSServer implements A4POSServer {
    ...
    */
   public A4ModelPOSServer(String inventoryFile) {
   ...
   public String createTransaction(TransactionType type, long time, int client) {
      ...
      switch (type) {
         case PURCHASE:
            t = new Purchase(tID, client, time);
            break;
         case RETURN:
            t = new Return(tID, client, time);
            break;
         case BACKORDER:
            t = new Backorder(tID, client, time);
            break;
      }
      lastID++;
   ...
   public String cancelTransaction(String id, long time, int client) {
      ...
      if (t == null){
         ...
         if(client != t.getClientID()){
            result = "Unable to cancel\nTransactions: " + t.getID() + " belongs to client " + t.getClientID();
         }
         else{
            // return the items to inventory if the cancelled transaction is purchase
            if(t.getType() == TransactionType.PURCHASE){
               for(TransactionUnit unit : t.getUnits().values()){
                  inventory.get(unit.getItem().getCode()).increaseInStock(t.removeItem(unit.getItem().getCode(), unit.getQuantity()));
               }
            }
